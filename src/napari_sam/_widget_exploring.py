...
class SamWidget(QWidget):
    ...
    def init_comboboxes(self):
        ...
        for layer_name in self.get_layer_names():
            ...
            @layer.events.name.connect # REDO meaning if you change the name of the layer this function will be called?
    ...
    # TODO rename because it is not used only on everything mode
    ...
    def _activate(self):
        ...
        if not self.is_active:
            self._initialize()
            self._handle_dim_is_3()
            self._validate_image_layer_ndim()
            self._initialize_sam_logits_if_ndim_2_else_as_list()
            self._set_annotator_mode()
            self._set_segments()
            self._configure_other_activations()
            self._configure_auto_mode_if_active()
        else:
            self._deactivate()
        self.btn_activate.setEnabled(True)

    def _initialize(self):
        self.image_name = self.cb_image_layers.currentText() #REDO
        self.image_layer = self.viewer.layers[self.cb_image_layers.currentText()]
        self.label_layer = self.viewer.layers[self.cb_label_layers.currentText()]
        self.label_layer_changes = None #REDO

    def _handle_dim_is_3(self):
        # fixes shapes if ndim is 3
        if self.image_layer.ndim != 3:
            return
        ...
        self._reset_scale_for_image_and_label_layer()
        self._set_viewer_point()

    def _reset_scale_for_image_and_label_layer(self):
        self.image_layer.affine.scale = np.array([1, 1, 1])
        self.image_layer.scale = np.array([1, 1, 1])
        self.image_layer.scale_factor = 1
        self.label_layer.affine.scale = np.array([1, 1, 1])
    ...
    def _set_viewer_point(self):
        pos = self.viewer.dims.point
        self.viewer.dims.set_point(0, 0)
        self.viewer.dims.set_point(0, pos[0])
        self.viewer.reset_view()

    def _validate_image_layer_ndim(self):
        if self.image_layer.ndim != 2 and self.image_layer.ndim != 3:
            raise RuntimeError("Only 2D and 3D images are supported.")

    def _initialize_sam_logits_if_ndim_2_else_as_list(self):
        if self.image_layer.ndim == 2:
            self.sam_logits = None
        else:
            self.sam_logits = [None] * self.image_layer.data.shape[0]

    def _set_annotator_mode(self):
        if self.rb_click.isChecked():
            self.annotator_mode = AnnotatorMode.CLICK
            self.rb_auto.setEnabled(False)
            self.rb_auto.setStyleSheet("color: gray")
    ...
    def _configure_other_activations(self):
        if self.annotator_mode != AnnotatorMode.AUTO:
            self._activate_non_auto_mode()

    def _activate_non_auto_mode(self):
        self.is_active = True
        self.btn_activate.setText("Deactivate")
        self._deactivate_non_relevant_buttons()
        self._configure_annotation_mode_and_layer()
        self._set_prev_segmentation_mode()
        self._configure_size_and_edge_width_based_on_image_ndim()
        self.create_label_color_mapping()
        self._configure_history_and_image()
        self._update_points_layer_and_callbacks()

    def _deactivate_non_relevant_buttons(self):
        self.btn_load_model.setEnabled(False)
        self.cb_model_type.setEnabled(False)
        self.cb_image_layers.setEnabled(False)
        self.cb_label_layers.setEnabled(False)
    ...
    def _configure_annotation_mode_and_layer(self):
        self.btn_mode_switch.setText("Switch to BBox Mode")
        self.annotator_mode = AnnotatorMode.CLICK
        selected_layer = None
        if self.viewer.layers.selection.active != self.points_layer:
        ...
        if selected_layer is not None:
            self.viewer.layers.selection.active = selected_layer
        self._handle_ndim_3_case()

    def _handle_ndim_3_case(self):
        if self.image_layer.ndim == 3:
            self._fix_bbox_visibility_and_viewer_point()

    def _fix_bbox_visibility_and_viewer_point(self):
        self.check_auto_inc_bbox.setChecked(False)
        self.update_bbox_layer({}, bbox_tmp=[[self.viewer.dims.current_step[0], 0, 0],
                                             [self.viewer.dims.current_step[0], 0, 10],
                                             [self.viewer.dims.current_step[0], 10, 10],
                                             [self.viewer.dims.current_step[0], 10, 0]])
        self.update_bbox_layer({}, bbox_tmp=None)
        self._set_viewer_point()

    def _set_prev_segmentation_mode(self):
        self.bbox_layer.editable = False
        self.bbox_first_coords = None
        self.prev_segmentation_mode = SegmentationMode.SEMANTIC

    def _configure_size_and_edge_width_based_on_image_ndim(self):
        if self.image_layer.ndim == 2:
            self._configure_for_ndim_2()
        else:
            self._configure_for_non_ndim_2()

    def _configure_for_ndim_2(self):
        self.point_size = int(np.min(self.image_layer.data.shape[:2]) / 100)
        if self.point_size == 0:
            self.point_size = 1
        self.bbox_edge_width = 10
    ...
    def _configure_for_non_ndim_2(self):
        self.point_size = 2
        self.bbox_edge_width = 1
        self.le_point_size.setText(str(self.point_size))
        self.le_bbox_edge_width.setText(str(self.bbox_edge_width))

    def _configure_history_and_image(self):
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=FutureWarning)
            self._history_limit = self.label_layer._history_limit
        self._reset_history()
    ...
    def _update_points_layer_and_callbacks(self):
        self.update_points_layer(None)
        self.viewer.mouse_drag_callbacks.append(self.callback_click)
        self.viewer.keymap['Delete'] = self.on_delete
        self.label_layer.keymap['Control-Z'] = self.on_undo
    ...
    def _configure_auto_mode_if_active(self):
        if self.annotator_mode == AnnotatorMode.AUTO:
            self._activate_auto_mode()

    def _activate_auto_mode(self):
        self.sam_anything_predictor = SamAutomaticMaskGenerator(
            self.sam_model,
            points_per_side=int(self.le_points_per_side.text()),
            points_per_batch=int(self.le_points_per_batch.text()),
            pred_iou_thresh=float(self.le_pred_iou_thresh.text()),
    ...
    # Switch between BBox and point Modes
    ...
    def do_point_click(self, coords, is_positive):
        ...
        # Save the history of the annotator
        self._save_history({
            "mode": AnnotatorMode.CLICK,
